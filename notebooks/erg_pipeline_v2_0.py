...
class ERGAudit:
    ...
    def _analyze_artifacts(self, signal: np.ndarray, fs_hz: float) -> Dict[str, Any]:
        ...
        freqs, psd = welch(signal, fs=fs_hz, nperseg=min(1024, len(signal)))
        emg_mask = (freqs >= 30) & (freqs <= 500)
        low_freq_mask = (freqs >= 10) & (freqs <= 50)

        if np.any(emg_mask) and np.any(low_freq_mask):
            emg_energy = np.sum(psd[emg_mask])
            signal_energy = np.sum(psd[low_freq_mask])
            emg_ratio = emg_energy / (signal_energy + 1e-30)
...
# Updated: Added OP extraction with OP4 window 35-65 ms, OP sum = OP2+OP3+OP4
# ============================================================================
...
class ERGFeatureExtractor:
    ...
    def extract_oscillatory_potentials(self, signal: np.ndarray, fs_hz: float,
        ...
        op_windows = {
            ...
            'OP2': (18, 26),
            'OP3': (24, 36),
            'OP4': (35, 65)  # Extended window per ISCEV
        }
...
# Add dynamic update function (place AFTER the widget definition)
...
# Attach the function to protocol_widget
